""" 
Copied from https://github.com/HazyResearch/state-spaces/blob/main/src/models/s4/s4.py
...
try:
    ...
    log.warning("FFTConv not being used.")
    fftconv_func = None
...
class SSKernelDiag(OptimModule):
    ...
    def __init__(
        ...
        # self.dt_rand = torch.rand_like(log_dt)
        # self.A_rand = torch.rand_like(A)
        # self.A_imag_rand = torch.rand_like(self.A_imag)
        # self.A_real_rand = torch.rand_like(self.A_imag)
        # self.dtA_rand = torch.rand_like(A)
        # self.B_rand = torch.rand_like(B)
        # self.C_rand = torch.rand_like(C)
    ...
    def _A(self):
        ...
        # A = self.A_real_rand + 1j * self.A_imag_rand
        return A
    ...
    def forward(self, L, state=None, rate=1.0, u=None):
        ...
        # DEBUG
        # A = self.A_rand.to(A.device)
        # dtA = self.dtA_rand.to(A.device)
        # A=A.real
        # B = self.B_rand.to(A.device)
        # C = self.C_rand.to(A.device)
...
class S4(nn.Module):
    def __init__(
        ...
        **kernel_args,
        ...
        # self.k_debug = nn.Parameter(torch.randn(self.H))
    ...
    def forward(self, u, state=None, rate=1.0, lengths=None, **kwargs):
        ...
        # ww = torch.exp(-1*torch.range(0,L-1)).view(1,1,-1).to(k.device)
        # k = torch.rand_like(k)*ww
        # k = self.k_debug.view(1,1,-1).to(k.device)
