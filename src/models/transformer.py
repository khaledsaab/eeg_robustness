...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_head, d_inner=2048, mha_cfg=None, attn_cfg=None, ffn_cfg=None,
        ...
        if mha_cfg is None:
            mha_cfg = {}
        if '_target_' in mha_cfg:
            self.self_attn = hydra.utils.instantiate(mha_cfg, d_model, n_head, batch_first=batch_first,
                                                     **factory_kwargs)
...
class TransformerClassifier(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, n_layer: int, d_inner: int, num_classes: int,
        ...
        super().__init__()
        assert pooling_mode in ['MEAN', 'SUM', 'CLS', 'LAST'], 'pooling_mode not supported'
