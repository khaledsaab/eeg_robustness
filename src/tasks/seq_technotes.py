...
class SequenceModel(LightningModule):
    ...
    def instantiate_datamodule(self):
        ...
        self._datamodule: LightningDataModule = hydra.utils.instantiate(
            self.cfg.datamodule
        )
        ...
        OmegaConf.register_new_resolver(
            "datamodule", lambda attr: getattr(self._datamodule, attr)
        )
    ...
    def instantiate_metrics(self):
        ...
        metrics = MetricCollection(
            {name: hydra.utils.instantiate(cfg) for name, cfg in metrics_cfg.items()}
        )
    ...
    def warmstart(self):
        if self.cfg.train.get("warmstart", None) is not None:
            logger.info(
                f"Warm-starting with weights from {self.cfg.train.warmstart.path}"
            )
            ...
            if self.cfg.train.warmstart.get("post_process", None) is not None:
                state_dict = hydra.utils.instantiate(
                    self.cfg.train.warmstart.post_process, state_dict
                )
    ...
    def step(self, batch: Any, is_train=True):
        ...
        if is_train:
            ...
            loss = self.loss_fn_val(output, y) #(output.softmax(1)[:, 0], y.float())
            
        # convert output and y to binary
        output = output.softmax(1)[:, 0]
        y = (y[:, 0] == 1).int()
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        loss, output, targets = self.step(batch, is_train=False)
    ...
    def configure_optimizers(self):
        if (
            "optimizer_param_grouping" in self.cfg.train
